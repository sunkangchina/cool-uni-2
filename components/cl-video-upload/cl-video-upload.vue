<template>
  <view class="cl-video-upload" :class="{ 'is-disabled': isDisabled }">
    ...
    <view class="video-list"> 
      ...
      <!-- 添加按钮 -->
      <view class="add-btn" v-if="showAddButton" :style="videoItemStyle" @click="selectVideo">
        ...
        handler(val) {
          this.$nextTick(() => {
            ...
            if (!val) {
              ...
              this.videoList = val.map(item => typeof item === 'string' ? {
                url: item
              } : item);
            } else if (isString(val)) {
              this.videoList = val.split(",").filter(Boolean).map(url => ({
                url
              }));
            } else if (val.url) {
              this.videoList = [val];
